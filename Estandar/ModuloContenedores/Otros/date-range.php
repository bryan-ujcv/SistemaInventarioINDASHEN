...
$query .= "SELECT `id`, `num_contenedor`, `chasis`, `placa_chasis`, DATE_FORMAT(`fecha_ingreso`,'%e/%M/%Y') as 'fecha_ingreso', `piloto_ingreso`, `placa_piloto_ingreso`, `empresa_ingreso`, DATE_FORMAT(`fecha_salida`,'%e/%M/%Y') as 'fecha_salida', `piloto_salida`, `placa_piloto_salida`, `empresa_salida`, `dias`, `genset`, `booking`, `tamano`, `ejes`, `observacion`, DATE_FORMAT(`hora_ingreso`,'%r') as 'hora_ingreso', DATE_FORMAT(`hora_salida`,'%r') as 'hora_salida',`tipo_tamano`,`destino` FROM `contenedores` WHERE `cliente`!='MAERSK' AND `fecha_salida` BETWEEN '$date1' AND '$date2'";
...
if (isset($_POST["date-repo"])) {
  ...
  $file_name = 'Historial Otros desde ' . $date1 . ' hasta ' . $date2 . '.xlsx';
