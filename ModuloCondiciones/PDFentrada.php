...
$sql2 .= "SELECT cond.id as ide, con.id as conid,con.num_contenedor as container, con.chasis as chasis, con.placa_chasis as placa, `tipo_condicion`,DATE_FORMAT(con.fecha_salida,'%e / %M / %Y') as fecha,con.piloto_salida as piloto,con.placa_piloto_salida as placa_piloto,con.empresa_salida as empresa 
    from condiciones as cond 
    join contenedores as con on con.id=contenedor_id
    where contenedor_id = '$idcon' and tipo_condicion='Entrada'";
    ...
    do {
        if ($result = mysqli_store_result($con)) {
            while ($rowi2 = mysqli_fetch_array($result)) {
                $pdf->Cell(40, 0, "Fecha : " . $rowi2['fecha'], 0, 0);
                $pdf->Cell(50);
                $pdf->Ln(10);
                ...
                $pdf->Cell(50, 5, "Cliente : Maersk", 1, 0);
                $pdf->Ln(10);
            }
        }
    } while (mysqli_next_result($con));
