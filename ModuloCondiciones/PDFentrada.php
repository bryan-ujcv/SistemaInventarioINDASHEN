...
$sql = "SELECT cond.id as ide, con.id as conid, con.num_contenedor as con, llanta1,llanta2,llanta3,llanta4,llanta5,llanta6,llanta7,llanta8,llanta9,llanta10,llanta11,llanta12,cond.observacion,firma_entrada
    from condiciones as cond 
    join contenedores as con on con.id=contenedor_id
    where contenedor_id = '$idcon' and tipo_condicion='Entrada'";
...
$sql2 .= "SELECT cond.id as ide, con.id as conid,con.num_contenedor as container, con.chasis as chasis, con.placa_chasis as placa, `tipo_condicion`,DATE_FORMAT(con.fecha_ingreso,'%e / %M / %Y') as fecha,con.piloto_ingreso as piloto,con.placa_piloto_ingreso as placa_piloto,con.empresa_ingreso as empresa,con.cliente as cliente 
    from condiciones as cond 
    join contenedores as con on con.id=contenedor_id
    where contenedor_id = '$idcon' and tipo_condicion='Entrada'";
...
if ($rowi['llanta9'] == '' && $rowi['llanta10'] == '' && $rowi['llanta11'] == '' && $rowi['llanta12'] == '') {
    if ($rowi['firma_entrada'] == null) {
        ...
        $pdf->Image('../CSS/IMG/condicion.jpg', 10, 180, 190);
        $pdf->Image('../CSS/IMG/firma.jpg', 92, 243, 30);
        $pdf->Image($rowi['firma_entrada'], 15, 244, 50);
    }
} else {
    if ($rowi['firma_entrada'] == null) {
        $pdf->Cell(40);
...
$pdf->Output('I', 'Condicion Entrada.pdf');
?>